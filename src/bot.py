...
async def remove_favorite(ctx):
    ...
    cursor.close()
    conn.close()
    await ctx.send("🗑️ Your favorite genre has been removed. Set a new one using `/setfavorite <genre>`.")
...
async def recommend(ctx):
    ...
    if not genre:
        return await ctx.send('⚠️ Please set a favorite genre first using `/setfavorite <genre>`.')
