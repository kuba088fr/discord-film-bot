...
class DummyCtx:
    def __init__(self, author_id=123, author_display_name="Tester"):
        self.author = type("Author", (), {"id": author_id, "display_name": author_display_name})()
        self.sent_messages = []
...
def mock_db_connection_for_bot_commands(monkeypatch):
    ...
    monkeypatch.setattr('src.bot.get_connection', MagicMock(return_value=mock_connection_instance))
...
async def test_setfavorite(mock_save_user_preference):
    ...
    mock_save_user_preference.assert_called_once_with(str(ctx.author.id), test_genre.lower())
    assert f"✅ Your favorite genre has been set to: **{test_genre}**" in ctx.last_sent_message
...
async def test_removefavorite(mock_db_connection_for_bot_commands):
    ...
    assert "🗑️ Your favorite genre has been removed. Set a new one using `/setfavorite <genre>`." in ctx.last_sent_message
...
async def test_recommend_with_preference(mock_get_random_movie, mock_get_user_preference):
    ctx = DummyCtx()
    ...
    assert f"🎬 Recommendation for **{user_favorite_genre}**: {recommended_movie_details}" in ctx.last_sent_message
...
async def test_recommend_no_preference(mock_get_user_preference):
    ...
    await bot.get_command("recommend").callback(ctx)
    mock_get_user_preference.assert_called_once_with(str(ctx.author.id))
    assert "⚠️ Please set a favorite genre first using `/setfavorite <genre>`." in ctx.last_sent_message
